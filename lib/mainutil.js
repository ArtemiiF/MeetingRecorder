...
// ── content fingerprint (H3b arch-audit — cache staleness) ──────────────────
// Cheap content fingerprint folded into cacheDirFor's cache-key tag (main.js):
// path+size+mtime ALONE (the previous tag) can't tell an in-place rewrite that
// preserves size and lands on a coarse/same-second mtime (a re-export, a sync
// tool, a same-second overwrite) from the original file — the pipeline would
// then silently reuse a transcript/diarization cache computed from the OLD
// content. Reads only the first+last 64KB (not the whole file — audio files run
// tens to hundreds of MB) — cheap, and catches the overwhelming majority of real
// edits (re-export, re-mix, truncation); a middle-only change with an unchanged
// head/tail/size is NOT detected — a deliberate, documented tradeoff (see the
// mainutil.test.js case for this), not a bug.
// Missing/unreadable file → "" — mirrors cacheDirFor's own try/catch around
// fs.statSync, which already tolerates a vanished audioFile the same way.
...
function contentFingerprint(filePath) {
  ...
  try {
    ...
    const headLen = Math.min(FINGERPRINT_CHUNK_BYTES, size);
    const head = Buffer.alloc(headLen);
    if (headLen) fs.readSync(fd, head, 0, headLen, 0);
    const tailLen = Math.min(FINGERPRINT_CHUNK_BYTES, size);
    const tailStart = Math.max(0, size - tailLen);
    const tail = Buffer.alloc(tailLen);
    if (tailLen) fs.readSync(fd, tail, 0, tailLen, tailStart);
    return crypto.createHash("sha1").update(head).update(tail).update(String(size)).digest("hex");
  } catch {
    return "";
  } finally {
...
// ── file-stability gate (H3a arch-audit) ─────────────────────────────────────
// Guards the "process-audio started before mixed.wav was fully written" incident
// (TODO.md — a 25-min recording processed as 0.4s because the pipeline's own
// mono-conversion cache snapshotted mixed.wav mid-write: 12KB captured while the
// real file was still growing to its final 48MB). Two samples of the SAME
// path's size, `waitMs` apart: unchanged size = stable (safe to process);
// changed = still being written by someone else, refuse.
// statFn(path) -> size in bytes, or null if the file doesn't exist/can't be
// stat'd (missing is never "stable enough to process" — refuses immediately,
// without even waiting, since there's nothing that could still be arriving).
// waitFn(ms) -> Promise, resolved after ms. Both injected so this stays testable
// without a real filesystem or a real timer — main.js's own caller passes
// fs.statSync-based/real-setTimeout-based implementations.
async function isFileStable(filePath, waitMs, statFn, waitFn) {
  const sizeBefore = statFn(filePath);
  if (sizeBefore == null) return false;
  await waitFn(waitMs);
  const sizeAfter = statFn(filePath);
  return sizeAfter != null && sizeAfter === sizeBefore;
}
...
//
// ── general path containment (H2 arch-audit) ─────────────────────────────────
// Generalizes the "is this resolved path inside one of my allowed roots?" check
// beyond isNoteDeletable/isAudioDeletable's note/audio-file-deletion use case —
// exported so main.js's OTHER renderer-supplied-path handlers (read-note,
// rename-speakers, reveal, and the --note/root/audio args of para-extract/
// para-classify/para-file) can validate a path against out_dir/the PARA vault
// root too, not just the delete handlers. Same discipline as isNoteDeletable's
// own resolvedPath contract: the CALLER resolves symlinks first (fs.realpathSync,
// catching the not-found/unreadable case as null) — this stays fs-free and
// unit-testable; a null resolvedPath always means "not inside anything".
function isPathInsideRoots(resolvedPath, roots) {
  if (!resolvedPath) return false;
  return (roots || []).some((root) => root && !isOutsideRoot(resolvedPath, root));
}
...
// Delegates to isPathInsideRoots (checking the file's directory, not the leaf name —
// mathematically equivalent for a plain file, see isPathInsideRoots' callers in main.js
// which check the resolved file path directly instead).
function _resolvedDirInsideRoots(resolvedPath, roots) {
  if (!resolvedPath) return false;
  return isPathInsideRoots(path.dirname(resolvedPath), roots);
}
...
// ── PARA filing (folder hierarchy) — T4-T6, ux-para-batch ───────────────────
// Sanitizes ONE path segment (never a whole joined path — main.js's para-file handler
// calls this per segment before path.join'ing them). Strips path separators (same
// characters the old single-segment sanitizer in main.js's para-file handler already
// stripped) so a hostile/weird project/person/mission string can only ever widen one
// folder name, never inject a "/" or ".." traversal segment of its own. Also strips
// LEADING dots — a dot-prefixed folder would be silently invisible to backend.py's own
// vault walk (_iter_vault_notes' `dirnames[:] = [d for d in dirnames if not
// d.startswith(".")]`, backend.py:1746, the same convention .trash already relies on to
// stay unindexed) — filing a note into one would make История quietly lose it.
function _sanitizeParaSegment(s, fallback) {
  const v = String(s || "").trim().replace(/[/\\:]/g, "-").replace(/^\.+/, "").trim();
  return v || fallback;
}

// Pure PARA destination-folder builder: given a classified note's category (+, for
// "projects", its kind/person/mission sub-classification from cmd_classify) and the
// recording's own stamp, returns the array of path segments (relative to the vault
// root) the note — and its audio — should be filed into. Never touches fs, never
// returns a filename: the caller (main.js's para-file handler) path.joins these onto
// the resolved vault root, mkdirs the result, and moves the RAW note there preserving
// its own `meeting-<stamp>*.md` basename (История's reconcile identity — see
// backend.py's _iter_vault_notes/_reconcile and this repo's CLAUDE.md) rather than
// distilling it into a single per-project accumulator file as before — chronology
// comes for free from the stamp-sorted basenames, so no accumulator is needed anymore.
//
// - archives/areas/resources → <CategoryFolder>/<год>/<месяц>/
// - projects, kind=one_to_one → <Projects>/один-на-один с <person>/
// - projects, kind=mission_daily → <Projects>/миссии/<год>. миссия <mission>/
// - projects, anything else → <Projects>/<project>/ (one folder per project, replacing
//   the old single accumulated <project>.md)
function paraDestinationDir({ category, folders, project, kind, person, mission, stamp } = {}) {
  const categoryFolder = (folders && folders[category]) || category || "Без категории";
  const m = String(stamp || "").match(/(\d{4})-(\d{2})-(\d{2})/);
  const year = m ? m[1] : "без даты";
  const month = m ? m[2] : "без даты";
  if (category === "projects") {
    const projectsFolder = _sanitizeParaSegment(categoryFolder, "Projects");
    if (kind === "one_to_one") {
      return [projectsFolder, _sanitizeParaSegment(`один-на-один с ${person || ""}`, "один-на-один")];
    }
    if (kind === "mission_daily") {
      return [projectsFolder, "миссии", _sanitizeParaSegment(`${year}. миссия ${mission || ""}`, `${year}. миссия`)];
    }
    return [projectsFolder, _sanitizeParaSegment(project, "Без названия")];
  }
  return [_sanitizeParaSegment(categoryFolder, category || "Без категории"), year, month];
}
...
// Permanently deletes one entry's files, skipping anything outside trashDir (belt-and-
// braces against a tampered/hand-edited manifest.json — same reasoning as purgeTrash's own
// inline check used to have). Factored out of purgeTrash so the Корзина tab's manual
// "Удалить навсегда"/"Очистить корзину" IPC handlers (main.js) can reuse the EXACT same
// containment-checked deletion instead of re-implementing it — "mirror purgeTrash's hard
// check" per the trash-tab feature spec. deps injected ({existsSync, unlinkSync}, default
// fs) for unit-testability, same convention as purgeTrash/moveToTrash. Never throws.
function deleteTrashEntryFiles(entry, trashDir, deps) {
  const d = deps || fs;
  for (const f of (Array.isArray(entry && entry.files) ? entry.files : [])) {
...
function purgeTrash(manifest, trashDir, maxAgeMs, now, deps) {
  ...
  for (const entry of list) {
    ...
    deleteTrashEntryFiles(entry, trashDir, deps);
  }
  return kept;
}

// Days left until purgeTrash's sweep (Корзина tab's "осталось N дн" pill) — ceil-rounded so any partial day left still reads ≥1, never 0.
function trashDaysLeft(deletedAt, nowMs, maxAgeMs = 30 * 24 * 3600 * 1000) {
  const dayMs = 24 * 3600 * 1000;
  return Math.max(0, Math.ceil((deletedAt + maxAgeMs - nowMs) / dayMs));
}

// Shared manifest-entry schema for delete-history-note/-recording (main.js) — adds id/title/origin, defaulting each when omitted.
function buildTrashEntry({ id, deletedAt, kind, files, baseStamp, title, origin }) {
  return {
    id,
    deletedAt,
    kind,
...
// Restore destination for one trashed file: an `origin` entry is honored only if it
// resolves inside `roots` (untrusted, hand-editable manifest.json content, and a
// vaultRoot reconfigured after the delete can strand an old origin outside the CURRENT
// roots too) — anything else falls back to `outDir/<basename>`, a best-effort default
// that's exactly right for outDir-originated files and merely acceptable (not "correct")
// for a legacy PARA-filed note, which reconcile still re-indexes via its own vault scan.
function restoreDestinationFor(filePath, origin, outDir, roots) {
  const mapped = origin && origin[filePath];
  if (mapped && isPathInsideRoots(mapped, roots)) return mapped;
  return path.join(outDir, path.basename(filePath));
}

// main.js's restore-trash-entry loop, extracted for electron-free unit-testing: moves
// each file to restoreDestinationFor's (containment-checked) target; a file that fails
// stays in `remaining` with its error reported, the others still restore. deps injected
// ({mkdirSync, existsSync, move}, default fs/moveToTrash) for testability.
function restoreTrashFiles(files, origin, outDir, roots, deps) {
  const d = deps || {};
  const mkdirSync = d.mkdirSync || fs.mkdirSync;
  const existsSync = d.existsSync || fs.existsSync;
  const move = d.move || moveToTrash;
  const remaining = [];
  let error = null;
  for (const f of (Array.isArray(files) ? files : [])) {
    try {
      const ideal = restoreDestinationFor(f, origin, outDir, roots);
      mkdirSync(path.dirname(ideal), { recursive: true });
      const dest = trashDestPath(path.dirname(ideal), path.basename(ideal), existsSync);
      move(f, dest);
    } catch (e) {
...
// Extension check for the Корзина tab's per-entry "аудио X МБ" byte split (main.js's
// list-trash handler sums audio-file bytes separately from note-file bytes). Single
// source of truth for "is this a trash-tracked audio file" — reuses the same extension
// set isAudioDeletable already validates against, so a new audio extension only ever
// needs to be added in one place.
function isTrashAudioFile(filePath) {
  return _TRASH_AUDIO_EXT.has(path.extname(filePath || "").toLowerCase());
}

// Per-entry audio/note byte+count split for list-trash's display (main.js) — every file
// is gated on a successful statSync first: a file that's already gone must not inflate
// noteCount/bytes with data from something that no longer exists. deps injected
// ({statSync}, default fs.statSync) for unit-testability.
function trashEntryBreakdown(files, deps) {
  const statSync = (deps && deps.statSync) || fs.statSync;
  let audioBytes = 0, noteBytes = 0, noteCount = 0;
  for (const f of (Array.isArray(files) ? files : [])) {
    let stat = null;
    try { stat = statSync(f); } catch {}
    if (!stat) continue;
    if (isTrashAudioFile(f)) audioBytes += stat.size;
    else if (f.endsWith(".md")) { noteBytes += stat.size; noteCount++; }
  }
  return { audioBytes, noteBytes, noteCount, bytes: audioBytes + noteBytes };
}
