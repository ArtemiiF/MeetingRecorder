...
// Days remaining before purgeTrashOnStartup's 30-day sweep permanently deletes an entry
// (Корзина tab's per-row "осталось N дн" pill — design ref: trash-tab-a.html). Rounds UP
// (Math.ceil) so a partial day remaining still reads "1", never "0", while anything is
// actually left — 0 is reserved for the point purgeTrash/isStale itself starts treating
// the entry as eligible for the sweep (elapsed >= maxAgeMs). now/maxAgeMs are parameters
// (not Date.now()/a hardcoded constant) for the same injectable-clock testability every
// other retention helper in this file already uses (isStale, purgeTrash).
...
// One manifest entry (loadTrashManifest/saveTrashManifest's { entries: [...] } shape),
// extended for the Корзина tab feature with `id` (stable identifier the restore/delete-
// one/empty IPC handlers key off of — legacy entries predating this feature get one
// backfilled on first list-trash read, see main.js), `title` (the История card's note
// title at delete time, plumbed through from the renderer — falls back to baseStamp/
// filename in the renderer/main.js display layer, not here) and `origin` (trash path →
// original absolute path, so restore knows exactly where a file came from; legacy
// entries have no origin — restoreDestinationFor below falls back to outDir for those).
// Factored out so delete-history-note and delete-history-recording (main.js) build this
// object identically instead of two divergent literals drifting apart.
...
// Restore destination for one trashed file (Корзина tab "↩ Восстановить"). A file moved
// into trash AFTER this feature shipped has a recorded `origin` entry — restores exactly
// there. A legacy file (moved before `origin` existed) has none: every existing delete
// flow (delete-history-note/-recording) only ever moved files OUT of outDir/the vault
// root INTO trash, so outDir is always a valid original home for ANY legacy file, note
// or audio — falling back to `outDir/<basename>` is correct, not a guess. Collision-safe
// suffixing on top of this ideal path is the caller's job (main.js reuses trashDestPath
// for that — same suffix-on-collision logic trash-ing already uses, just pointed at a
// different destination directory).
function restoreDestinationFor(filePath, origin, outDir) {
  return (origin && origin[filePath]) || path.join(outDir, path.basename(filePath));
}
