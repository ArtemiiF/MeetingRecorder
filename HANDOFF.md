...
- Remote `ArtemiiF/MeetingRecorder` (private, solo). `main` = origin/main = `a1e7569` (readiness, PR #6). Тег `v1.0.0` = релиз с .dmg (перенесён в публичный репо, sha переписан). Фича-ветки спрюнены.
