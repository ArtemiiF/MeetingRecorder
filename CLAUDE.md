...
## Артефакты (claude.ai) — визуальные документы процесса

Артефакты — приватные веб-страницы на claude.ai; re-publish по тому же URL = новая версия (история версий сохраняется). Не источник правды — код первичен; line-якоря снимать на конкретном SHA и фиксировать его в футере страницы.

- **Схема архитектуры** — https://claude.ai/code/artifact/c6589f95-071a-424b-b567-a34dcf792265 (топология, IPC-группы, пайплайн+кэши, хранилища, чистые ядра, тест-слои). **Обновлять при каждом структурном сдвиге** (новый вид, IPC-группа, изменение этапов пайплайна, рефактор ядер) — шаг закрытия сессии вместе с HANDOFF.md.
- **PR-walkthrough** — для батча ≥3 PR или одиночного рискованного PR: артефакт-разбор для ревью владельцем перед мержем — что менялось (по PR), риски, находки критик-гейтов, диаграмма затронутых подсистем.
- **Инцидент-страница** — когда класс инцидентов набрал ≥2 повтора (образец класса: asar-паковка, 7 багов): таймлайн, root cause по каждому, поставленные локи (тесты/правила). Дешевле раскопок по хронике HANDOFF.
